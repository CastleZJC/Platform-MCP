...
**当前状态（V3.0 M0-M6 全部落地，2026-09-05；2026-09-08 增补附件与广场版本归档；2026-09-09 双端口径修订——个人设置/改密移出 MCP 归 Web，个人 Skill 启停入 MCP set_my_skill_status，装饰器/广场过审（origin=PLAZA）仅 admin Web 调整；2026-09-11 Skill 生命周期增强——迭代通知〔无邮件〕/ merge 工作台 2 工具 / 广场回滚 API / README 迭代段落 / 多语言 extra 分级补档 / 个人库重命名与复制冲突二选一 / Web 待审双视图与文件预览；2026-09-13 依赖安全升级 A 类 10 项已应用——mcp 1.30.0 / starlette 1.6.0 / FastAPI 0.141.1 / pydantic 2.13.5 / uvicorn 0.52.4 / python-multipart 0.0.32 / sqlparse 0.6.0 / py7zr 1.1.3 / aiomysql 0.3.2 / 前端 axios 1.20.0，C 类 7 项遗留三期见下方三期遗留段）**：33 MCP tools（database 5 + server 6 + skill 生态/双通道 20 + 双端承接 2；ToolMeta.roles 三角色过滤 admin 33 / developer 30 / 一般用户 19）；系统表 29 张（Alembic head=014）；三角色（admin / developer / user）；i18n 中/英（前端 vue-i18n 全站 key 化 + 后端 RESOURCES 55 key）；运行时配置中心（KNOWN_KEYS 注册表 + 30s 快照缓存 + 热切换）；Skill 生命周期 8 状态 varchar 状态机 + 版本化双语存档 + 广场（BGE-M3 语义搜索 / 黑名单 / admin 停用）+ 本地生成模型栈（Qwen3-4B 纯 CPU，模板兜底）+ 分享迭代 diff；MCP 双通道附件（create/update_my_skill readme + attachments，与 Web 上传同限）+ get_skill_file 包内文件下发；广场版本文件归档（pmcp_plaza_version + _plaza_versions 快照 + 手工回退脚本）；list_datasources/list_servers 响应含 groups；邮件组提醒 ×4（outbox 模式）；三期 KB 骨架（5 表 + RAG/GRAPH 抽象 + api 501 占位）。

**Tests**: 1721 backend pytest（`--ignore=tests/performance` 门禁口径）+ 202 frontend vitest + mypy 0 errors (112 files)。POC verification tests remain in `poc/`（本地专用，未入库）。
...
| `platform_mcp.skills.audit` / `readme` / `upload` / `versioning` | 14 条合规审计引擎、README 生成、Skill 包上传链路、版本化双语存档（generated_by 留痕）、merge 工作台（`merge_service`）、README 迭代段落（`iteration_readme`）、个人库重命名（`personal`，2026-09-11） |
...
- **V3.0 Skill 生命周期增强（2026-09-11，migration 014，工具 31→33）**：广场版本链 +patch（source_version 存提交人版本）/ 迭代通知（广场版本变更→origin=PLAZA 副本 SHARE_ITERATION，无邮件）/ merge 工作台（`build_merge_version` + `publish_merge_version` 仅 admin）/ 回滚开放（`POST /plaza/{id}/versions/{version}/rollback`）/ README 迭代段落（幂等标记 + 启动补历史）/ 多语言分级补足（readme_extra/report_extra，tier template<model<external）/ 个人库补齐（重命名 / 复制冲突二选一 10006 / Web 启停状态机统一 / 黑名单推荐过滤）/ Web 审核布局（待审双视图 + 文件预览 + name_match 同名比对）
...
- **审计 51 处 `write_audit_log`**（M5 起邮件捕捉点经其单一咽喉路由，PROD+HIGH/CRITICAL db/server 操作 Web+MCP 双入口）。
...
- **内置 Skill 启动自动同步**（2026-09-07）：Web 启动时 `skills/bootstrap.py` 按 registry `BUILTIN_SKILL_CODES` 把装饰器注册的 Skill 自动落/刷新 `pmcp_skill`（插入 ENABLED/decorator/tool_count 实测；已有行仅刷新 tool_count，不覆盖 admin 停用状态与用户编辑）；接入指南 `/guide/tools` 由此全量展示（5 skill / 33 tools），功能描述按登录 locale 经 `skill.desc.*` 取值；指南页使用建议为前端 i18n 静态文案（`/guide/usage` 端点已删）。
...
- **Skill 附件与广场版本归档**（2026-09-08）：MCP `create_skill_draft` / `update_my_skill` 支持 `readme` 原文 + `attachments: [{path, content_base64}]`（总量 ≤ `skill.max_upload_size_mb` 与 Web 上传同限热切换，路径仅包内相对路径禁穿越，附件进 14 条审计扫描，checksum 计入附件字节；附件须与 skill_md 一同传入覆盖式重 archived）；`get_skill_file`（skill_id|plaza_id + path）按可见性下发包内文件（文本 utf-8 / 二进制 base64，CC 取 SKILL.md 正文与 references 的唯一通道）；**注册链路绝对路径自动调整**（`skills/normalize.py`，Web/MCP 双通道，先于审计与校验和——可解析到包内文件的改包内相对（reference）、其余工作路径改 `./<basename>`（workdir），调整记录 `path_adjustments` 随版本 `audit_snapshot` 落库，历史版本不受影响）；**文件级版本归档仅限广场 Skill**（migration 013 `pmcp_plaza_version` + `_plaza_versions/{plaza_id}/{version}/` 全量快照 + file_manifest，approve/merge 落档、启动 `backfill_plaza_versions` 存量补版，回滚 2026-09-11 已服务化开放——`POST /plaza/{id}/versions/{version}/rollback`〔admin〕+ PlazaPage 版本列表弹窗，脚本壳保留；个人/装饰器系统 Skill 仅最新版）；`list_datasources`/`list_servers` 响应条目含 `groups`（manager 层 `resource_group_names`，过滤语义不变）；工具描述双语并列分隔符必须「中文 / English」（空格三字符，`split_bilingual` 按 CJK 边界拆分）；decorator+template 存档启动自愈刷新 readme_en（external/model 产物永不覆盖）。
- **Skill 生命周期增强**（2026-09-11，migration 014）：① **迭代通知无邮件**——广场版本任何变更（过审发布/merge 发布/重发布/回滚）→ 所有 origin=PLAZA 个人副本（ENABLED/DISABLED，黑名单豁免、含提交人）经状态机批量动作 MARK_ITERATION 置 SHARE_ITERATION，非阻断（MCP 仍可用），`mark_plaza_holders_for_iteration` 单咽喉；② **merge 工作台仅 admin**——build（文件级并集+主源优先+冲突清单+临时包+14 条审计预跑）→ `get_skill_file(merge_token)` 试用 → publish（🔴 终审 critical>0 阻断；升广场新版+快照归档+README 迭代段落+持有者迭代标记）/discard；**原创 Skill 并入广场走工作台（原 origin=PLAZA 限制废除），`review_skill action=merge` 快捷路径仅限已关联副本**；③ **版本链**——广场版本自增 +patch（首发=提交人版本），提交人版本存 `pmcp_plaza_version.source_version`；复制记 `copied_from_plaza_version`（老版本 3-way 基线）；④ **回滚不新建版本行**（内容=归档版），同走迭代标记；⑤ **多语言 tier template<model<external，本地永不覆盖 external**——`pmcp_skill_version.readme_extra/report_extra` JSONB，`submit_skill_artifact` 增 `content_extra {locale:text}`，`get_skill_readme` 按 locale 分级解析（主列→extra→中文回退）；⑥ **个人库**——重命名条件（非装饰器∧origin≠PLAZA∧未分享∧稳定态，磁盘目录同步改名，MCP `update_my_skill.skill_code` + Web `PUT /skills/{id}`，owner-only）；add-to-my 复制冲突二选一（**10006**，overwrite 覆盖保留行 / retry+new_code，`{code}-{username}` 派生码退役）；Web `PUT /skills/{id}/status` 委托 `set_enabled`（owner 或 admin；内置/广场复制仍仅 admin 直写）；⑦ **Web 审核布局**——SkillPage 双视图（待审提交 admin lazy）+ `GET /skills/{id}/files` 文件预览（防穿越）+ `compute_name_match` 同名比对（merge / reject_ref / merge_candidate）。
...
11. **生产发布四段式验证（强制）**：每次生产发布（除纯文档/纯 README 更新外）必须严格执行以下四段式流程，缺一不可：
    - **段一 预检（本地）**：跑全量回归 `pytest tests/ --ignore=tests/performance -q`（期望 1721 passed）+ `mypy platform_mcp/`（0 errors / 112 files，需安装 dev 依赖含 `types-PyYAML` 存根）+ `cd platform-mcp-frontend && npx vue-tsc -b`（exit 0）+ `npx vitest run`（202 passed）。**全绿才能进入段二**，任一红立即终止并修代码。
