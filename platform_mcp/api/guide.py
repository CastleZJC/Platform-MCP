...
async def get_usage(_user: dict = Depends(get_current_user)):
    ...
    return ResponseBase(data={
        "scenarios": [
            {
                "title": "模糊匹配数据源（SQL）",
                "user_says": "用 176 执行 documents/tst/x.sql",
                "behavior": "Claude 查询 list_datasources，按 176 子串匹配 datasource_code / name / host；唯一命中直接执行，多匹配时弹出候选让用户选择",
            },
            {
                "title": "显式选择数据源（SQL）",
                "user_says": "执行 documents/tst/x.sql",
            ...
            {
                "title": "模糊匹配服务器（Shell）",
                "user_says": "在 176 上执行 uname -a",
                "behavior": "Claude 查询 list_servers，按 176 子串匹配 server_code / name / host；唯一命中直接执行 execute_command，多匹配时弹候选",
            },
            {
                "title": "上传/下载文件（SFTP）",
                "user_says": "把 D:\\pkg\\x.tar.gz 传到 176 的 /tmp/",
            ...
            },
            {
                ...
                "user_says": "176 上查 Oracle 的 dual 表 / 176 上看磁盘空间",
                "behavior": "Claude 依据意图自动选 execute_sql_text 或 execute_command；模糊场景（如『176 上检查一下』）Claude 会反问用户：『是查 Oracle 数据库还是看服务器磁盘？』，确认后再调用对应 skill",
            },
        ],
        "tips": [
            "数据源/服务器关键字优先用编码片段（如 176、app-sample-2），匹配精度高于主机名",
            ...
            "Claude 自动意图识别：依据用户语义选择 database/server skill；当指令模糊（如『检查一下 176』『修复 216』）时反问用户，避免误用 skill",
