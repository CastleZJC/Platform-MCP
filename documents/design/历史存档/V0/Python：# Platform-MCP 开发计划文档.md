...
> |------|----------|----------|----------|--------|
> | v20260605140000 | 2026-06-05 14:00:00 | 文档审核修复 | 补全 5 张缺失系统表；统一 confirm_token 机制；技术栈表补全+版本精确化；明确页面数量；补充 passlib/CBC/版本约束/Node.js/前置条件 | castle.zhang |
> | v20260605170000 | 2026-06-05 17:00:00 | 全量文档交叉审核修复 | A1 confirm_token 四方统一；A2 响应格式补齐 trace_id/timestamp；B1-B6 Alembic版本/技术栈补4依赖/common覆盖率/性能测试/部署配置/Node.js同步；C1-C5 CBC代码/mypy/延后标注 | castle.zhang |
> | v20260606090000 | 2026-06-06 09:00:00 | 全量文档交叉审核修复（第三轮） | B1 删除过时注释；B2 修正 §5.5/§5.6 章节编号；B4 §5.1 验收标准补齐 auth/common ≥90%；C2 文档索引补充正式版架构说明 | castle.zhang |
> | v20260611090000 | 2026-06-11 09:00:00 | 第一轮一致性校验 | 前端版本对齐实际（Vue 3.5.34 / TypeScript 6.0.2 / Vite 8.0.12）；补充 mypy 到质量门禁依赖 | castle.zhang |
> | v20260611130000 | 2026-06-11 13:00:00 | 第二轮一致性校验 | 实现异步执行功能（§10.5）；添加 MCP 权限控制（operator_role + 环境限制）；CORS 外部化；SQL 文本大小限制（1MB）；前端类型定义修复；标注细粒度数据源权限为二期 | castle.zhang |
> | v20260605090000 | 2026-06-05 09:00:00 | 模型与 Skill 调整 | AI 模型切换为 GLM 5.1；推荐工具列替换为 ecc/superpowers 具体技能 | castle.zhang |
> | v20260603090000 | 2026-06-03 09:00:00 | 初始创建 | 基于技术架构文档，建立一期开发计划 | castle.zhang |
>
...
| 交付物 | 说明 |
|--------|------|
| MCP Server | stdio 模式，Claude Code 子进程调用，支持 5 个 Tool |
| FastAPI Web | REST API 管理端，8 个核心页面 |
| Database Skill | SQL 文本执行、SQL 文件执行、风险识别、数据源列表、执行状态查询 |
| PostgreSQL 系统库 | 用户/角色/权限/数据源/审计日志/Skill 注册 |
| 部署方案 | 单机 systemd + Nginx，Rocky Linux 9.4 |
...
| 数据校验 | Pydantic + pydantic-settings | 2.8.2 / 2.4.0 |
...
| 1.1.3 | 搭建 7 模块骨架目录（api / auth / datasource / mcp_server / skills.database / audit / common） | GLM 5.1 | ecc:python-patterns | ✅ 完成 |
...
**验收标准**：
- `alembic upgrade head` 成功创建全部 13 张系统表（pmcp_user、pmcp_role、pmcp_user_role、pmcp_permission、pmcp_role_permission、pmcp_api_key、pmcp_datasource、pmcp_datasource_permission、pmcp_audit_log、pmcp_mcp_call_log、pmcp_crypto_operation_log、pmcp_skill、pmcp_system_config）
...
| 4.2.3 | 前端：登录页（双栏布局 + 角色选择下拉框） | GLM 5.1 | ui-ux-pro-max | ✅ 完成 |
...
**验收标准**：
- Oracle 11g：5 个 Tool 全部通过
- MySQL 5.6：5 个 Tool 全部通过
- 目标库异常时 Web 管理端正常响应
...
| # | 任务 | AI 模型建议 | 推荐工具/Skill | 状态 |
|---|------|------------|---------------|------|
| 5.6.1 | 前端构建产物打包（`npm run build`） | GLM 5.1 | ecc:deployment-patterns | 待执行 |
| 5.6.2 | 后端部署脚本编写（systemd service 文件） | GLM 5.1 | ecc:deployment-patterns | 待执行 |
| 5.6.3 | Nginx 反向代理配置 | GLM 5.1 | ecc:deployment-patterns | 待执行 |
| 5.6.4 | PostgreSQL 配置调优（生产级） | GLM 5.1 | ecc:postgres-patterns | 待执行 |
| 5.6.5 | Secret 文件生成与权限设置 | GLM 5.1 | ecc:security-review | 待执行 |
| 5.6.6 | Oracle Instant Client 安装配置 | GLM 5.1 | ecc:deployment-patterns | 待执行 |
| 5.6.7 | 服务启动与健康检查验证 | GLM 5.1 | superpowers:verification-before-completion | 待执行 |
| 5.6.8 | Claude Code 接入 MCP Server 验证 | GLM 5.1 | superpowers:verification-before-completion | 待执行 |

**验收标准**：
- `GET /api/v1/health` 返回 `{"status": "UP"}`
- Claude Code 可通过 MCP 调用全部 5 个 Tool
- 日志输出正常，无 ERROR 级别异常
...
| # | 验收项 | 验收方式 |
|---|--------|---------|
| C1 | Oracle 11g 基础连接和 SQL 执行通过 | 5 个 Tool 全量调用 |
| C2 | MySQL 5.6 基础连接和 SQL 执行通过 | 5 个 Tool 全量调用 |
