...
|---|---|---|---|---|
| V1.0 | 2025-08-08 00:00:00 | 新建 | 初版技术架构说明文档，基于 Java 技术路线，补充详细技术栈、模块设计、接口边界、部署方案、运维要求、测试基线与实施约束 | castle.zhang |
...
| 组件 | 推荐版本 | 说明 |
|---|---|---|
| JDK | 17.0.12 LTS | 长期支持版本，兼顾稳定性与生态成熟度 |
| Spring Boot | 3.2.8 | 当前长期稳定、企业可用性高；若 Oracle 驱动适配受限，可回退 2.7.18 |
...
| HikariCP | 5.0.1 | 连接池，稳定成熟 |
| PostgreSQL JDBC Driver | 42.7.3 | PostgreSQL 系统库连接驱动 |
| MySQL Connector/J | 5.1.49 | 针对 MySQL 5.6 的稳定兼容版本 |
| Oracle JDBC Driver | ojdbc8 19.22.0.0 / ojdbc6 11.2.0.4 | 需根据 JDK 和 Oracle 11g 实际适配验证 |
...
## 6.1 Spring Boot 版本建议
默认推荐：
- **Spring Boot 3.2.8**

但考虑到 Oracle 11g 驱动及个别旧环境兼容性，项目评审时应保留以下策略：

### 优先策略
- 以 Spring Boot 3.2.8 + JDK 17 为正式目标版本

### 回退策略
- 若 Oracle JDBC 驱动联调验证存在不可接受风险，则回退至：
  - Spring Boot 2.7.18
  - Spring Security 5.8.x
  - 保持 JDK 17 或降至 JDK 11
...
- JDK 17 + `ojdbc8 19.22.0.0`
- 若存在适配问题，再验证：
  - JDK 8/11 + `ojdbc6 11.2.0.4`

项目正式启动前，应以验证结果固化最终驱动版本。
...
- `Platform-MCP-web-admin`
- `Platform-MCP-auth`
- `Platform-MCP-datasource`
- `Platform-MCP-crypto`
- `Platform-MCP-mcp-core`
- `Platform-MCP-skill-api`
- `Platform-MCP-skill-registry`
- `Platform-MCP-skill-database`
- `Platform-MCP-sql-executor`
- `Platform-MCP-risk-engine`
- `Platform-MCP-audit`
- `Platform-MCP-monitor`
- `Platform-MCP-common`
...
### 7.2.7 Platform-MCP-skill-registry
负责：
- Skill 注册发现
- Tool 与 Skill 映射
- Skill 执行入口管理
...
### 7.2.8 Platform-MCP-skill-database
负责：
- 数据库 Skill 业务逻辑
- Tool 能力落地
- SQL 文件/文本执行组织
...
### 7.2.9 Platform-MCP-sql-executor
负责：
- SQL 执行
- 多语句分段
- 查询结果映射
...
### 7.2.10 Platform-MCP-risk-engine
负责：
- SQL 风险识别
- 高危语句标记
- 语法解析失败记录
...
- Oracle 11g JDBC 驱动最终版本
- MySQL 5.6 驱动版本
- JDK 与 Spring Boot 最终组合
- SQL 文件路径读取策略
...
- **后端**：Java 17.0.12 + Spring Boot 3.2.8  
