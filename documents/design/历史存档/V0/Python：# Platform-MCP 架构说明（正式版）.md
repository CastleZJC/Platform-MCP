...
Web 管理端采用 Session 方案（用户名密码登录，Session 存储于 PostgreSQL），选择 Session 而非 JWT 的原因：内部系统简单稳定，后台管理场景更易控制会话失效管理。登录页采用双栏布局：左侧为企业项目视觉区（项目名称、功能亮点），右侧为登录表单（含角色选择下拉框），底部版权说明。
