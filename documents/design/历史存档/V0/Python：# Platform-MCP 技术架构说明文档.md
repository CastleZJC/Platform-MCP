...
| 版本 | 日期 | 修订性质 | 修订摘要 | 修改人 |
|---|---|---|---|---|
| V1.0 | 2026-05-28 | 新建 | 初版技术架构说明文档，整合架构设计与三份技术评审共识，确立 Python 3.11.9 技术路线为项目启动基线 | castle.zhang |
| V1.1 | 2026-05-29 | 修订 | 新增 Skill 管理页与个人设置页；密码加解密页更名为密码加密页；补充 Skill 注册方式说明；登录页改为双栏布局；补充 pmcp_skill 系统表 | castle.zhang |
| V1.2 | 2026-06-03 | 修订 | POC 验证结论同步：Oracle 驱动策略从 thin async 切换为 thick 模式 + run_in_executor（thin 模式不支持 11g，thick 6/6 通过）；MySQL aiomysql 14/14 项全量通过；双角色模型（admin/developer）与 Skill 审核流程定稿；新增 MCP 接入指南页；PROD 数据源仅 admin 可调用 | castle.zhang |
...
| 模块 | 包含能力 | 说明 |
|---|---|---|
| `platform_mcp.api` | FastAPI Web 接口 | 前端对接接口与页面数据聚合输出 |
| `platform_mcp.auth` | 认证鉴权 | 登录认证、用户/角色/权限管理 |
| `platform_mcp.datasource` | 数据源管理 + 密码加解密 | 数据源配置、环境管理、密码加密解密 |
| `platform_mcp.mcp_server` | MCP 接入 + Skill 接口 + 注册路由 + Skill 管理 | MCP 协议接入、Skill 统一接口定义、Skill 注册与路由分发、Skill 管理与生命周期 |
| `platform_mcp.skills.database` | 数据库 Skill + SQL 执行器 + 风险引擎 | 数据库 Skill 业务逻辑、SQL 执行、风险识别 |
...
- 登录页采用双栏布局：左侧为企业产品视觉区（产品名称、功能亮点），右侧为登录表单，底部版权说明。角色由用户名自动映射（admin/developer 双角色绑定用户账号），无需用户手动选择
...
共 13 张系统表（实测 `__tablename__` 全列表）。其中 `pmcp_datasource_permission` 和 `pmcp_system_config` 为一期已建未用资产（ORM 已定义，业务逻辑/API 留待二期实现）。
...
**设计原则：** datasource_code 和 env_code 作为可选字段，确保审计结构面向所有 Skill 通用。extra_data 使用 JSONB 类型存储，为二期新增 Skill 预留扩展空间。
...
- 登录页（双栏布局：左侧产品视觉区 + 右侧登录表单，角色由用户名自动映射）
