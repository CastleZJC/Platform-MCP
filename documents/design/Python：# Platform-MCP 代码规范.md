...
├── mcp_server/      # MCP 协议接入、Skill 路由、双传输（stdio + streamable-http）
├── skills/
│   ├── database/    # 数据库 Skill（SQL 执行、风险识别）
│   ├── server/      # 服务器 Skill（SSH/SFTP 执行、Shell 风控）
│   └── common/      # Skill 共用：RiskLevel/RiskResult + 环境权限校验
├── audit/           # 审计日志、调用统计
└── common/          # 异常、响应模型、枚举、工具类
```

**依赖方向**：`api → auth / datasource / server / skills → audit → common`，禁止反向依赖。
