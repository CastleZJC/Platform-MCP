...
- 服务器 SSH 密码与 PEM 私钥（`pmcp_server.encrypted_password` / `pmcp_server.encrypted_ssh_key`）
- API Key 明文（`pmcp_api_key.key_encrypted`，admin reveal 用）
- 加密密钥本身的管理与保护
- 审计日志中禁止记录明文密码
...
```yaml
datasource:
  crypto_key_path: <deploy_root>/secret/crypto-secret.key   # dev: 项目根；prod: /opt/Platform-MCP/secret/ 或 /opt/<user>/mcp_app/app/secret/
```
...
`.gitignore` 实际排除项（与仓库根 `.gitignore` 对齐）：

```
*.key                        # 所有密钥文件（含 crypto-secret.key）
.env                         # 环境变量文件
settings.yml                 # 主配置（含开发态覆盖）
settings-prod.yml            # 生产配置
*.bak / *.tmp / *.orig       # 备份/临时文件
```

> **注**：仓库实际 `.gitignore` 未列 `secret/` 与 `settings-dev.yml`（前者通过 `*.key` 已覆盖；后者当前不存在，dev 直接用 `settings.yml`）。如未来拆分 `settings-dev.yml`，需手动追加规则。
...
1. 生成新密钥（32 raw bytes，**严禁 base64 编码**——`CryptoUtils.__init__` 强制 `len(key) == 32`）：`openssl rand -out crypto-secret.key 32` 或 `head -c 32 /dev/urandom > crypto-secret.key`
...
### 5.X crypto-secret.key 跨环境隔离原则（V1.0 强制）

**核心原则**：每环境（dev/test/prod）独立 crypto-secret.key，**绝不跨环境共享或拷贝**。

| 项 | 要求 |
|---|---|
| 生成 | 每环境独立执行 `openssl rand -out crypto-secret.key 32` 生成 32 字节随机二进制 |
| 权限 | `chmod 600` + `chown <user>:<user>` |
| 路径 | dev: 项目根；prod: `/opt/Platform-MCP/secret/crypto-secret.key`（不进 git，`.gitignore` 已通过 `*.key` 规则排除） |
| 共享 | **禁止**跨环境拷贝 key 文件 |
| 迁移 | 跨环境迁移加密数据时**必须**用源 key 解密 → 目标 key 重新加密（不传输明文，详见《部署规范.md》§五） |
| 轮换 | 每半年评估；轮换时需 re-encrypt 全部密文（pmcp_datasource.encrypted_password + pmcp_server.encrypted_password + pmcp_server.encrypted_ssh_key + pmcp_api_key.key_encrypted） |

**违规后果**：跨环境共享 key 会导致一个环境泄露 → 所有环境密文可被解密，违反最小权限原则。

**审计要点**：
- key 文件不应在 git 历史中出现（`git log --all --full-history -- '*.key'` 应为空）
- 不同环境的 `crypto-secret.key` `sha256sum` 必须不同
- 部署脚本禁止 `scp crypto-secret.key user@remote:`
