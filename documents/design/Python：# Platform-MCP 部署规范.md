...
sudo -u platform_mcp PLATFORM_MCP_ENV=prod /opt/Platform-MCP/venv/bin/python -m alembic upgrade head
...
sudo -u platform_mcp PLATFORM_MCP_ENV=prod /opt/Platform-MCP/venv/bin/python -m alembic downgrade -1
systemctl start Platform-MCP Platform-MCP-mcp
```
...
│   ├── config/
│   │   ├── settings.yml              # 主配置
│   │   └── settings-prod.yml         # 生产覆盖（PLATFORM_MCP_ENV=prod 加载）
...
```bash
cd $APP
PLATFORM_MCP_ENV=prod venv/bin/python -m alembic upgrade head
# seed（首次）：scripts/_setup_local.py 已支持 trust 认证
PLATFORM_MCP_ENV=prod venv/bin/python scripts/_setup_local.py
...
- `config.py`（`PLATFORM_MCP_ENV` 加载机制已就绪）
...
PLATFORM_MCP_ENV=prod $APP/venv/bin/python -m alembic upgrade head
bash $APP/run/start_all.sh
...
PLATFORM_MCP_ENV=prod $APP/venv/bin/python -m alembic downgrade -1
bash $APP/run/start_all.sh
```
