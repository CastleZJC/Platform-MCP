...
| group（V2.1 起） | 80% | 90% | 分组管理（V3.0 统一组迁移需含回填校验用例） |
| skills.audit / skills.readme / skills.upload（V2.1 起） | 80% | 90% | 合规审计引擎 / README 生成 / 上传链路 |
| i18n（V3.0 规划） | 80% | 90% | 多语种字典与本地化返回 |
| notify（V3.0 规划） | 80% | 90% | 邮件组 + outbox（SMTP 用本地 mock，不联真发） |
| review（V3.0 规划） | 80% | 90% | 可复用审核流状态机 |
| skills.llm（V3.0 规划） | 特殊口径 | — | 模型推理本身不进 CI：测试走**模板兜底路径 + llama-cpp/fastembed 接口 mock**；权重文件不进仓库不进 CI（无权重时兜底链路必须全过）；`generated_by` 留痕断言 |
...
| **覆盖率是下限，不是免测额度** | 达到覆盖率门禁后**不得以此为由跳过任何可测节点**；以**业务场景清单法**为准：列出功能全部可测节点（正常路径/边界/异常/权限/状态转移/双端入口），逐节点映射用例，未映射节点即漏测，必须补齐（2026-09-02 强化，V3.0 M0 起对一期/V2.1 存量一并复查） |
| 实现方式符合原有设计 | Mock、Stub、Fixture 必须尊重被测代码的真实依赖结构；**测试适配代码，不是代码迁就测试** |
| 不脱离原有业务场景 | 测试输入、调用路径、断言点必须可追溯到需求文档、API 契约或实际调用方 |
| **反装饰性验证** | 凡有前端展示/写库状态的功能，须有测试证明其实际业务效果（写库状态有消费方、启停真实生效），防止"仅一个装饰作用"（V3.0 起，对应开发计划 F-43/F-44） |
...
### 7.0 V1.0 类型检查（CI 必过项）

V1.0 引入 mypy==1.11.2 作为后端类型守门，与 pytest 并列：
...
Server Skill 单元测试位于 `tests/unit/test_server_*.py`：
