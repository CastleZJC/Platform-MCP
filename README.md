...
- **运行时配置中心（V3.0 M1）**：已知键注册表（13 键，生效语义 relogin/immediate/仅新用户生效，凭证值不回显）+ 30s 快照缓存，默认语言/新用户默认每页条数/会话失效时间/日志级别等非重启生效项统一由系统配置页管理；个人每页条数（5/10/20/50/75/100）在个人设置即时调整、全部列表页生效（分页组件全站统一）
