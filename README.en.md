...
> Dual Skills: Database (SQL execution) + Server (Linux SSH/SFTP) — 33 MCP tools in total (incl. Skill plaza / dual-channel ecosystem & merge workbench), invoked remotely by callers such as Claude Code, equipped with a Web management portal and full-chain audit
...
- **Skill ecosystem (since V2.1, V3.0 enhancements)**: user skill upload & registration (14-rule compliance audit + 8-state lifecycle + versioned bilingual archive) + plaza sharing (review flow / copy-to-my / blacklist / admin merge workbench / versioned file archive & rollback) + MCP dual-channel creation and iteration (package file delivery via `get_skill_file`, iteration diff and external-artifact submission)
...
**Backend**: Python 3.11.9 + FastAPI 0.141.1 + SQLAlchemy 2.0.35 + Alembic 1.13.2 + oracledb 2.4.1 + aiomysql 0.3.2 + asyncssh 2.17.0 (Server Skill SSH/SFTP) — dependency security upgrade batch 2026-09-13, see `README.md` version table (V3.0.1) and `documents/bug/BUG20260913001000`

**Frontend**: Vue 3.5.34 + Vite 8.0.12 + TypeScript 6.0.2 + Element Plus 2.8.1 + Pinia 2.2.2 + Axios 1.20.0 + vue-i18n ^9.14.5 (Chinese/English bilingual)
...
│   │   ├── audit/               # 14-rule compliance audit engine (V2.1; content sanitization removed)
...
├── tests/                       # Backend tests (1721 cases)
...
| `CLAUDE.md` § Documentation Review Standards | Documentation review rules (embedded in CLAUDE.md) |
| `documents/ui/Platform-MCP-portal.html` | UI prototype (single-file HTML) |

## Version Iteration

> **Baseline V1.0 = 2026-08-08**. Every subsequent production release (including hotfixes, iteration versions, and config-change rollouts) must append a row to this table — see `CLAUDE.md §部署原则 #10`. Row order follows milestone batches before dates (e.g. the V2.1 batch row precedes later-dated hotfix rows); the `pyproject.toml` `version` is a shorthand (3.0.x) and does not map 1:1 to milestone rows.

| Version | Date | Type | Summary | Author |
|------|------|------|------|--------|
| V3.0.1 | 2026-09-13 | Iteration | Dependency security upgrade batch (`documents/bug/BUG20260913001000`): **Class-A 10 items fixed** — backend mcp 1.9.4→1.30.0 (GHSA-jpw9-pfvf-9f58 etc., chose the last 1.x for compatibility), starlette→1.6.0, FastAPI 0.115.0→0.141.1, pydantic 2.8.2→2.13.5, uvicorn[standard] 0.30.6→0.52.4, python-multipart 0.0.9→0.0.32, sqlparse 0.5.0→0.6.0, py7zr 0.22.0→1.1.3, aiomysql 0.2.0→0.3.2, frontend axios 1.7.4→1.20.0; **Class-C 7 items decided to defer to phase 3** (cryptography ≥49.0.0 / aiosmtplib ≥5.1.2 / asyncssh 2.x latest / vitest 5 / model stack llama-cpp-python+fastembed / MCP description-injection governance / element-plus ≥2.11.1, see architecture §19.6.1). Wheelhouse offline package list synchronized (added py7zr / aiosmtplib). Regression: pytest 1721 / mypy 0 (112 files) / vitest 202 / vue-tsc 0 / build passed | castle |
| V3.0 Enh | 2026-09-11 | Iteration | Skill lifecycle enhancements (migration 014, 7 batches): **(1) Version chain & iteration notice** — plaza versions auto-increment +patch (first release = submitter's version, admin publish may override; `pmcp_plaza_version.source_version` keeps the submitter's source version); any plaza version change (review publish / merge publish / republish / rollback) → all origin=PLAZA copies (ENABLED/DISABLED, blacklist-exempt incl. the submitter) transitioned to SHARE_ITERATION via the state machine MARK_ITERATION action (no email, non-blocking); review email template adds skill_id/submitter/version parameters. **(2) Merge workbench** — `pmcp_plaza_merge` slim table + `skills/merge_service.py` (file-level union + primary-source priority + conflict list with per-file admin adjudication + 14-rule audit pre-run + 🔴 final-review blocking; temp package `_plaza_merge/{pid}/{token}`; five scenarios: original folded-in [origin=PLAZA restriction abolished] / latest-version iteration / old-version 3-way [copied_from_plaza_version baseline] / two-user iteration / admin self-held) + MCP `build_merge_version`/`publish_merge_version` (admin only, tools 31→**33**, matrix admin 33/developer 30/user 19; `get_skill_file` extended with merge_token trial) + Web `/plaza/merge/*` 4 endpoints + SkillPage workbench dialog (review_skill quick-merge limited to already-linked copies). **(3) README iteration section** — idempotent `<!-- PMCP_ITERATION_SECTION -->` appended to all READMEs: plaza = authoritative chain (one real-file diff per PUBLISHED version + rollback tail row), personal origin=PLAZA inherits on iteration reset / origin=ORIGINAL lists its own version history; startup `backfill_readme_iterations` backfills history. **(4) Rollback opened** — `POST /plaza/{id}/versions/{version}/rollback` (admin, serviced rollback_plaza_version: snapshot restore + README entry + holder iteration marking, no new version row) + PlazaPage version-list dialog. **(5) Multilingual tiered fill** — `pmcp_skill_version.readme_extra/report_extra` JSONB; tier template<model<external (local never overwrites external); `submit_skill_artifact` adds `content_extra {locale:text}`; `get_skill_readme` tiered locale resolution (main column → extra → Chinese fallback); MCP response artifact_hint; new languages → system republish to backfill history (deployment requirement). **(6) Personal library completion** — rename (PUT /skills/{id} + update_my_skill.skill_code, conditions: non-decorator ∧ origin≠PLAZA ∧ unshared ∧ stable state, disk directory renamed in sync); add-to-my copy conflict two-choice (error code 10006, overwrite/retry, derived codes retired); Web enable/disable delegates to the set_enabled state machine (owner or admin; built-in/plaza copies still admin direct-write); blacklist recommendation filtering (scan_plaza_similar single throat). **(7) Web review layout** — SkillPage dual view "My Skills / Pending Reviews (admin, lazy)" + review-dialog file preview (GET /skills/{id}/files path-traversal-safe) + compute_name_match same-name adjudication (merge/reject_ref/merge_candidate). Gates: pytest 1721 / mypy 0 (112 files) / vitest 202 / vue-tsc 0 / build passed | castle |
...
| V1.0 | 2026-08-08 | Baseline release | Phase 1 + Server Skill phase-2 dedicated rollout full launch: 11 MCP tools (database 5 + server 6), 15 system tables, 9 frontend pages, dual-transport MCP (stdio + streamable-http), API Key dual storage (hash + encrypted) | castle |

## Testing

```bash
# Backend (1721 cases, --ignore=tests/performance scope)
python -m pytest tests/ --ignore=tests/performance --cov=platform_mcp
mypy platform_mcp/    # Type checking (added in V1.0, 112 files 0 errors)
