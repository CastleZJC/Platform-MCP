...
- **Runtime config center (V3.0 M1)**: known-key registry (14 keys, effect semantics relogin/immediate, sensitive keys masked + second confirmation) + 30s snapshot cache; non-restart items such as default locale / session timeout / log level are managed uniformly on the system config page
...
```bash
# Backend (900 cases, --ignore=tests/performance scope)
python -m pytest tests/ --ignore=tests/performance --cov=platform_mcp
mypy platform_mcp/    # Type checking (added in V1.0, 80 files 0 errors)
